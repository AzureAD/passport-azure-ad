...
Strategy.prototype.authenticate = function(req, options) {
  var self = this;
    ...
    var policy = req.query.p;
        ...
        this._options.identityMetadata = this._options.identityMetadata.replace("common", this._options.tenantName);
        this._options.identityMetadata = this._options.identityMetadata.concat('?p=' + policy);

        log.info('B2C: New Metadata url provided to Strategy was: ', this._options.identityMetadata);
...
// 
self.setOptions(self._options, function(err, options) {
          ...
          if (load) {
            ...
            if (idToken) {
              ...
              try {
                ...
                profile.email = jwtClaims.upn || jwtClaims.preferred_username;
 ...
 */
 Strategy.prototype.setOptions = function(options, done) {
var self = this;
...
// 
 async.waterfall([
    ...
    log.info("Metadata we have is: ", options.identityMetadata);
    metadata = new Metadata(options.identityMetadata, "oidc");

    metadata.fetch(function(err) {
        if (err) {
            return next(new Error("Unable to fetch metadata: " + err));
        } else {
